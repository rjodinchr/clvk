...
static void cvk_api_command_buffer_reset_state(cl_event event,
...
struct cvk_api_command_buffer : public _cl_command_buffer_khr,
    ...
    cl_int finalize() {
        ...
        for (auto queue : m_queues) {
            for (auto cmd : m_commands[queue]) {
                cmd->set_part_of_api_command_buffer();
            }
        }
    ...
    cl_int enqueue(const std::vector<cvk_command_queue*>& queues,
        ...
        for (unsigned i = 0; i < queues.size(); i++) {
            if (queues[i]->properties() != m_queues[i]->properties()) {
